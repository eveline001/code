...
void ChainNegate(int u, int v){
    ...
    if(tp[u] == tp[v]){
        if(id[u] > id[v]){
            Negate(id[v] + 1, id[u], 1, n, 1);
        }else{
            Negate(id[u] + 1, id[v], 1, n, 1);
        }
        return;
    }
    if(de[tp[u]] < de[tp[v]]){
...
int ChainQuery(int u, int v){
    ...
    if(tp[u] == tp[v]){
        if(id[u] > id[v]){
            return Query(id[v] + 1, id[u], 1, n, 1);
        }else{
            return Query(id[u] + 1, id[v], 1, n, 1);
        }
    }
    if(de[tp[u]] < de[tp[v]]){
...
int ChainQuery(int u, int v){
    ...
    while(tp[u] != tp[v]){
        if(de[tp[u]] < de[tp[v]]) swap(u, v);
        res = max(res, Query(id[tp[u]], id[u], 1, n, 1));
        u = fa[tp[u]];
    }
    if(u == v) return res;
    if(de[u] > de[v]) swap(u, v);
    return max(res, Query(id[u] + 1, id[v], 1, n, 1));
}
