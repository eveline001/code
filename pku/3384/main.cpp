...
void NarrowAddLine(Line& L, double x1, double y1, double x2, double y2, double len){
    double a = x1 - x2,
           b = y1 - y2,
           c = sqrt(a * a + b * b);
    double detX = b / c * len,
           detY = a / c * len;
    L.a = Point(x1 + detX, y1 - detY);
    L.b = Point(x2 + detX, y2 - detY);
...
int main()
{
    ...
    while(scanf("%d%d", &n, &r) != EOF){
        ...
        for(int i = 1; i < n; i++){
            scanf("%d %d", &x2, &y2);
            NarrowAddLine(L[tol++], x2, y2, x1, y1, r);
            x1 = x2, y1 = y2;
        }
        NarrowAddLine(L[tol++], x0, y0, x1, y1, r);
        HalfPlaneIntersect();
        maxdis = 0.0;
        Pa = P[0], Pb = P[0];
        for(int i = 0; i < pn - 1; i++){
            for(int j = i + 1; j < pn; j++){
                curdis = sqrt((P[i].x - P[j].x) * (P[i].x - P[j].x) + (P[i].y - P[j].y) * (P[i].y - P[j].y));
                if(curdis > maxdis) Pa = P[i], Pb = P[j], maxdis = curdis;
            }
        }
        printf("%.4f %.4f %.4f %.4f\n", Pa.x, Pa.y, Pb.x, Pb.y);
    }
