...
int ChainQuery(int a, int b, int n){
    ...
    if(tp[a] == tp[b]){
        if(id[a] > id[b]){
            return Query(id[b] + 1, id[a], 1, n, 1);
        }else{
            return Query(id[a] + 1, id[b], 1, n, 1);
        }
    }
    if(de[tp[a]] < de[tp[b]]){
        return Query(id[tp[b]], id[b], 1, n, 1) + ChainQuery(a, fa[tp[b]], n);
    }else{
        return Query(id[tp[a]], id[a], 1, n, 1) + ChainQuery(fa[tp[a]], b, n);
    }
}
...
int ChainQuery(int a, int b){
    int tp1 = tp[a], tp2 = tp[b], res = 0;
    while(tp1 != tp2){
        if(de[tp1] < de[tp2]){
            swap(tp1, tp2), swap(a, b);
        }
        res += Query(id[tp1], id[a], 1, n, 1);
        a = fa[tp1];
        tp1 = tp[a];
    }
    if(a == b) return res;
    if(id[a] > id[b]) swap(a, b);
    return res + Query(id[a] + 1, id[b], 1, n, 1);
}
